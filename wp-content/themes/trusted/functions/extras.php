...
/**
 * Move individual homepage sections active/inactive options
 * to one option for new sortable/activate setting
 */
function trusted_check_homepage_sections() {

	if ( !get_theme_mod('home_order') ) {

		if ( !get_theme_mod('front_page_content_off') ) {
			$trusted_hp_content = '1';
		} else {
			$trusted_hp_content = '0';
		}

		set_theme_mod('home_order',  array('tabs' => 'featured:'.get_theme_mod('featured_enable','0').',about:'.get_theme_mod('about_enable','0').',shop:'.get_theme_mod('woo_home_enable','0').',content:'.get_theme_mod('front_page_content_off',$trusted_hp_content).',posts:'.get_theme_mod('recent_posts_enable','0').',cta:'.get_theme_mod('cta_enable','0').'' ) );

	}

}
add_action( 'after_setup_theme', 'trusted_check_homepage_sections' );
...
function trusted_dynamic_style( $css = array() ){
	...
	if ($font_headings && $font_headings != 'Montserrat:regular,700' ) {
		$css[] = 'h1,h2,h3,h4,h5,h6,.page .woocommerce-tabs .wc-tabs li a,.woocommerce div.product .woocommerce-tabs .wc-tabs li a,.wc-block-grid__product-title{font-family:\''.trusted_css_font_family($font_headings).'\', Helvetica, Arial, Verdana, sans-serif;}';
	}
	...
	if ($sec_color && $sec_color != "#4f5e70") {
		...
		$css[] = '#masthead,.main-excerpt p,.taxonomy-description p,.term-description p,.page-description p{background:rgba( '.$sec_color_rgb['r'].', '.$sec_color_rgb['g'].', '.$sec_color_rgb['b'].',0.8);}#cta-section{background-color:rgba( '.$sec_color_rgb['r'].', '.$sec_color_rgb['g'].', '.$sec_color_rgb['b'].',0.8);}.featured-post .featured-icon{box-shadow: 0px 0px 0px 4px rgba( '.$sec_color_rgb['r'].', '.$sec_color_rgb['g'].', '.$sec_color_rgb['b'].',0.45);}';
	}
...
function trusted_block_editor_dynamic_style( $css = array() ) {

	$font_page_title = get_theme_mod( 'font_page_title', 'Ubuntu:300,300italic,regular,italic,500,500italic,700,700italic' );
	if ($font_page_title && $font_page_title != 'Ubuntu:300,300italic,regular,italic,500,500italic,700,700italic' ) {
		$css[] = '.editor-post-title__block .editor-post-title__input{font-family:\''.trusted_css_font_family($font_page_title).'\', Helvetica, Arial, Verdana, sans-serif;}';
	}

	$font_content = get_theme_mod( 'font_content', 'Source+Sans+Pro:300,400,600,700' );
	if ($font_content && $font_content != 'Source+Sans+Pro:300,400,600,700' ) {
		$css[] = '.editor-default-block-appender textarea.editor-default-block-appender__content,.editor-styles-wrapper p,.editor-styles-wrapper ul,.editor-styles-wrapper li{font-family:\''.trusted_css_font_family($font_content).'\', Helvetica, Arial, Verdana, sans-serif;}';
	}

	$font_headings = get_theme_mod( 'font_headings', 'Montserrat:400,700' );
	if ($font_headings && $font_headings != 'Montserrat:400,700' ) {
		$css[] = '.editor-styles-wrapper h1,.editor-styles-wrapper h2,.editor-styles-wrapper h3,.editor-styles-wrapper h4,.editor-styles-wrapper h5,.editor-styles-wrapper h6{font-family:\''.trusted_css_font_family($font_headings).'\', Helvetica, Arial, Verdana, sans-serif;}';
	}

	$heading_color = get_theme_mod( 'heading_color', '#323b45' );
	if ($heading_color && $heading_color != "#323b45") {
		$css[] = '.editor-styles-wrapper h1,.editor-styles-wrapper h2,.editor-styles-wrapper h3,.editor-styles-wrapper h4,.editor-styles-wrapper h5,.editor-styles-wrapper h6{color: '.esc_attr($heading_color).';}';
	}

	$body_text_color = get_theme_mod( 'body_text_color', '#323b44' );
	if ($body_text_color && $body_text_color != "#323b44") {
		$css[] = '.editor-styles-wrapper p,.editor-styles-wrapper ul,.editor-styles-wrapper li{color: '.esc_attr($body_text_color).';}';
	}
					...
					if ( class_exists( 'WooCommerce' ) ) {
						...
						$woo_account_page_id = get_option( 'woocommerce_myaccount_page_id' );
							...
							if ( is_user_logged_in() ) {
								...
								$woo_account_login_title = trusted_translation_wc( 'login' );
								$woo_account_reg_title = trusted_translation_wc( 'register' );
...
/**
 * Homepage Sections
 * Output the homepage sections in the chosen order
 */
if(!function_exists('trusted_home_order')){
	function trusted_home_order() {
		$home_order_tabs = get_theme_mod( 'home_order' );
		if ( $home_order_tabs['tabs'] != '' ) {
			$home_tabs = trusted_home_order_sections();	
			$tabs = explode( ',', $home_order_tabs['tabs'] );
			$section_number = 0;
			foreach ($tabs as $tab) {
				...
				if ($tab_active == 1) {
					if ($tab_id == 'featured') {
						trusted_featured_services($section_number);
					} elseif ($tab_id == 'about') {
						trusted_about_section();
					} elseif ($tab_id == 'shop') {
						trusted_woo_tabs();
					} elseif ($tab_id == 'content') {
						trusted_home_order_content();
					} elseif ($tab_id == 'posts') {
						trusted_home_recent_posts();
					} elseif ($tab_id == 'cta') {
						trusted_home_order_cta();
					}
				}
			}
		} else {
...
if(!function_exists('trusted_home_order_content')){
	function trusted_home_order_content() {
...
if(!function_exists('trusted_home_order_cta')){
	function trusted_home_order_cta() {
		echo '</div><!-- .container -->';
		trusted_cta_section();
...
if(!function_exists( 'trusted_featured_services' )){
	function trusted_featured_services($section_number) {

	if ($section_number == 1) {
		$fps_margin_class = '';
	} else {
		$fps_margin_class = ' top-margin';
	}
	$enable_featured_link = get_theme_mod( 'enable_featured_link', true);
?>
	<section id="featured-post-section" class="section<?php echo $fps_margin_class; ?>">
		<?php
		if ( get_theme_mod( 'animate_on' ) ) { ?>
 ...
 */
if(!function_exists( 'trusted_home_recent_posts' )){
	function trusted_home_recent_posts(){
	?>
	<section id="recent-posts-section" class="section clearfix">
		<h2 class="recent-posts-title"><?php echo get_theme_mod( 'recent_posts_title', esc_html__( 'From Our Blog', 'trusted' ) ) ?></h2>
		<div class="home-posts-wrap">
		<?php
				$trusted_home_recent_posts_args = array(
					'numberposts' => '3',
					'orderby' => 'post_date',
					'order' => 'DESC',
					'post_type' => 'post',
				...
				$home_recent_posts = wp_get_recent_posts( $trusted_home_recent_posts_args );
				foreach( $home_recent_posts as $home_recent ){

					?>
					<div class="home-post">						
						<a href="<?php echo esc_url( get_permalink( $home_recent["ID"] ) ); ?>"><?php echo get_the_post_thumbnail( $home_recent["ID"], 'large' ); ?>
						<h3><?php echo get_the_title($home_recent["ID"]); ?></h3></a>
						<div class="home-post-excerpt">
						<?php
						$home_recent_excerpt = apply_filters( 'the_excerpt', get_post_field( 'post_excerpt', $home_recent["ID"] ) );
						if ( $home_recent_excerpt == '' ) {
							$home_recent_excerpt = '<p>' . wp_trim_words( strip_shortcodes( get_post_field( 'post_content', $home_recent["ID"] ) ), 15 ) . '</p>';
						}
						if ( $home_recent_excerpt != '' && $home_recent_excerpt != '<p></p>' ) {
							echo $home_recent_excerpt;
						}
						?>
						<a href="<?php echo esc_url( get_permalink( $home_recent["ID"] ) ); ?>" class="button featured-readmore"><?php echo esc_html__( 'Read More', 'trusted' );?></a>
						</div>
					</div>
					<?php
				}
 ...
 */
if(!function_exists( 'trusted_home_order_sections' )){
	function trusted_home_order_sections(){
		$tabs = array();
		$tabs['featured'] = array(
			'id'       => 'featured',
			'label'    => esc_html__( 'Featured Services', 'trusted' ),
			'callback' => 'trusted_featured',
		);
		$tabs['about'] = array(
			'id'       => 'about',
			'label'    => esc_html__( 'About Section', 'trusted' ),
			'callback' => 'trusted_about',
		);
		if ( class_exists( 'WooCommerce' ) ) {
			$tabs['shop'] = array(
				'id'       => 'shop',
				'label'    => esc_html__( 'Shop Tabs', 'trusted' ),
				'callback' => 'trusted_shop',
			);
		}
		$tabs['content'] = array(
			'id'       => 'content',
			'label'    => esc_html__( 'Page Content', 'trusted' ),
			'callback' => 'trusted_content',
		);
		$tabs['posts'] = array(
			'id'       => 'posts',
			'label'    => esc_html__( 'Recent Posts Section', 'trusted' ),
			'callback' => 'trusted_posts',
		);
		$tabs['cta'] = array(
			'id'       => 'cta',
			'label'    => esc_html__( 'Call-to-action Panel', 'trusted' ),
			'callback' => 'trusted_cta',
		);
		return apply_filters( 'trusted_home_order_sections', $tabs );
	}
}
...
/**
 * Exclude custom logo from Jetpack lazy load, to counter conflict with #masthead height
 */
if(!function_exists( 'trusted_exclude_class_from_lazy_load' )){
	function trusted_exclude_class_from_lazy_load( $classes ) {
		$classes[] = 'custom-logo';
		return $classes;
	}
}
add_filter( 'jetpack_lazy_images_blacklisted_classes', 'trusted_exclude_class_from_lazy_load', 999, 1 );
